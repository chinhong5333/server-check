...
describe("raw load history", () => {
  ...
  beforeEach(() => {
    ...
    execute.mockImplementation(async (sql: string) => {
      if (sql.includes("FROM agents a")) return [[{
        ...
        latest_storage_total_bytes: "100000000000", latest_storage_available_bytes: "60000000000", latest_storage_mount_point: "/data",
        ...resourceOverrides
      }], []];
  ...
  it("separates raw bucket averages from the latest heartbeat and retains normalized history", async () => {
    ...
    expect(execute.mock.calls[0][0]).toContain("ORDER BY f.available_bytes / f.total_bytes ASC");
