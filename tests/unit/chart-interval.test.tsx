...
it("defaults each chart to 30 minutes without extra requests and keeps selections independent", async () => {
  fetchMock.mockResolvedValue({ points: [{ at: 2 }] });
  ...
  await waitFor(() => expect(a.result.current.points).toEqual([{ at: 2 }]));
  ...
  expect(a.result.current.points).toBe(points);
});
it("rejects unsupported intervals and ignores late responses after switching", async () => {
  ...
  await waitFor(() => expect(view.result.current.points).toEqual([{ at: 3 }]));
  await act(async () => { complete({ points: [{ at: 99 }] }); });
  expect(signal.aborted).toBe(true);
  expect(view.result.current.points).toEqual([{ at: 3 }] );
});
