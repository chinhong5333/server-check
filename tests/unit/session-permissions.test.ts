...
const state = vi.hoisted(() => ({ permissions: '["view_projects","delete_agents"]', disabled: 0 }));
vi.mock("../../src/server/db", () => ({ getPool: () => ({ execute: async () => [[{
  ...
  role:"sub_admin",permissions_json:state.permissions,is_disabled:state.disabled,csrf_hash:"unused",expires_at:Date.now()+3600000,revoked_at:null,last_seen_at:Date.now()
}]] }) }));
...
beforeEach(() => { state.permissions='["view_projects","delete_agents"]';state.disabled=0; });
it("uses current persisted role and grants instead of stale JWT authority", async () => {
  ...
  state.disabled=1;
  expect((await request(app).get("/delete").set("Cookie",cookie)).status).toBe(401);
  state.disabled=0;
